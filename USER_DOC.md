...
but you also can check out the `./srcs/docker-compose.yml` file for more information about services under the `services` key.
...
for the administration panel you need the `WP_USER` you set up in the file located at `./srcs/.env` and the password which is expected to be at `./secrets/wp_admin_password.txt`.
if you have aquired those with no problem you can just visit `https://[yourhost|localhost]/wp-admin` and login normally.

# Locate and manage credentials.
DOTENV file that is responsible for usenames, hosts, other information is located at: `./srcs/.env` in this format:
    `
        MARIA_DATABASE_NAME=database
        MARIA_DB_USER=db_user
        MARIA_DB_HOST=dbhost:dbport
        WP_USER=db_user
